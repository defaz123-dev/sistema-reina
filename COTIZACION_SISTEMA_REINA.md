# COTIZACIÓN DE SISTEMA DE GESTIÓN COMERCIAL - SISTEMA REINA (PREMIUM ENTERPRISE)

**Para:** Negocio con Gestión de Sucursales y Facturación SRI  
**De:** Christian Defaz - Consultoría de Software  
**Fecha:** 7 de marzo de 2026  
**Vigencia:** 30 días  

---

## 1. DESCRIPCIÓN DE LA SOLUCIÓN
Propuesta integral para la implementación del **Sistema Reina (Versión Enterprise)**, un ecosistema diseñado para el control total de operaciones comerciales, gestión multisucursal y cumplimiento tributario automatizado.

### Pilares Tecnológicos:
*   **Facturación Electrónica SRI (Full Pro):** Generación, firma digital XAdES-BES, envío automático y representación RIDE en formato A4 profesional. Incluye motor de reintentos y sincronización inteligente.
*   **Arquitectura Multisucursal:** Control independiente de puntos de emisión y establecimientos. Cada local factura con su propio secuencial y serie legal.
*   **Business Intelligence & Dashboard:** Analítica avanzada con 7 reportes de decisión (Rentabilidad Real, Ticket Promedio, Horas Pico, Insumos Críticos).
*   **POS Multicanal Delivery:** Gestión de precios diferenciados por plataforma (Local vs PedidosYa vs Uber Eats) con cambio de tarifa en tiempo real.
*   **Seguridad Bancaria:** Cifrado de firmas electrónicas, protección de sesiones y registro ininterrumpido de auditoría validado para cada acción del usuario.

---

## 2. INVERSIÓN (ACTUALIZADA)

Dada la envergadura del motor de facturación y la capacidad multisucursal, se presentan las siguientes opciones:

### OPCIÓN A: PROPIEDAD TOTAL (LICENCIA PERPETUA)
*Para empresas que desean el control total de sus datos y servidor.*

| Concepto | Detalle | Total |
| :--- | :--- | :---: |
| **Licencia Perpetua** | Versión ENTERPRISE (SRI + Sucursales + BI) | $ 1,850.00 |
| **Configuración SRI** | Parametrización de firma, ambientes y series | $ 300.00 |
| **Capacitación Pro** | Formación en gestión de costos y auditoría | $ 200.00 |
| **TOTAL INVERSIÓN INICIAL** | | **$ 2,350.00** |
| *Soporte y SRI Update* | *Garantía de cumplimiento SRI y Soporte VIP* | *$ 350.00/año* |

### OPCIÓN B: MODELO SAAS (MENSUALIDAD)
*Ideal para un inicio rápido con soporte continuo incluido.*

| Concepto | Detalle | Costo |
| :--- | :--- | :---: |
| **Suscripción Mensual** | Servidor Cloud + Mantenimiento + SRI Update | $ 120.00/mes |
| **Activación Pro** | Configuración de entorno y carga de recetas | $ 250.00 |
| **TOTAL INICIAL** | **Primer mes + Activación** | **$ 370.00** |

---

## 3. VENTAJAS COMPETITIVAS
1.  **Independencia Operativa:** Facture desde cualquier sucursal con datos reales y legales al instante.
2.  **Control de Márgenes:** Sepa exactamente cuánto dinero gana por cada producto vendido tras descontar el costo de insumos.
3.  **Cero Errores SRI:** Sistema validado que previene duplicados y errores de firma electrónica.
4.  **Marca Blanca Total:** Personalice el sistema con sus colores, logotipos y nombre comercial.

---

## 4. TÉRMINOS Y CONDICIONES
*   **Tiempo de Implementación:** 7 a 10 días laborables (incluye validación de firma con SRI).
*   **Requerimientos:** Certificado de firma electrónica (.p12) y RUC activo.
*   **Forma de Pago:** 50% al aceptar la propuesta y 50% tras la capacitación final.

Atentamente,

**Christian Defaz**  
Desarrollador Senior de Sistemas
